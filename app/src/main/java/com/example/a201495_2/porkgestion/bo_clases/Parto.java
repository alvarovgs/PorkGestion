...
public class Parto {
    ...
    public ArrayList<Parto> getAllPartoByView(){
        dbAcces = new dataBaseOpenHelper(appContext);
        String strSql = "SELECT IDPARTO, PARTO.IDCERDO,  FECHAPARTO,LECHONESVIVOSMACHOS,LECHONESVIVOSHEMBRAS,LECHONESMUERTOSMACHOS,LECHONESMUERTOSHEMBRAS,INDICEMORTALIDAD,PROMEDIOPESO,CERDO.CODIGO FROM PARTO INNER JOIN CERDO ON CERDO.IDCERDO=PARTO.IDCERDO";
        Cursor crResult;
        ArrayList<Parto> listObject = new ArrayList<Parto>();
        dbAcces.openDataBase();
        crResult = dbAcces.qweryDatabaseBySql(strSql);
